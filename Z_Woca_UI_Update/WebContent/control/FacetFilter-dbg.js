 ...
 *----------------------------------------------------------------------*/ 
	(function() {
    "use strict";
    jQuery.sap.declare("com.zespri.awct.control.FacetFilter");
    jQuery.sap.require("com.zespri.awct.control.FacetFilterCheckboxList");
    ...
    /**
     * @classdesc The FacetFilter is extended so that we can use our custom renderer for it. This is needed, since only the <i>FacetFilterList</i>s with the
     * custom property <i>showInFacetFilterBar</i> need to be rendered in the facet filter bar. Other lists need to be visible only when the user
     * clicks on the 'more filter items' button in the facet filter bar.
     * 
     * Only modifying the renderer or extending the control wasn't enough, as more modifications were required in the control. The code has been
     * copied from the standard implementation of sap.m.FacetFilter, and some custom changes have been done on top of that. These changes are marked
     * with the "CUSTOM CODE START" and "CUSTOM CODE END" comments.
     * 
     * A custom property 'filtersModifiedAfterListOpen' has been introduced to this control. This is updated by FacetFilterList (and custom lists)
     * whenever the user interacts with them. This helps in avoiding unnecessary roundtrips to the server if the user just opens a FacetFilterList and
     * then closes it without making changes. Since this property is set only when the user actually interacts with a FacetFilterList, it can be used
     * to determine whether any filters have changed. It is reset to false when the 'Reset' button is clicked, and when a list is opened. The
     * controller is expected to check for this property when the listClose event is fired. A better design would be to pass an extra parameter with
     * the 'listClose' event, indicating whether filters were changed.. however, this isn't standard behaviour - and would require copying the
     * standard FacetFilterList and making it a custom control (which is to be avoided unless absolutely necessary).
     * @class 
     * @name com.zespri.awct.control.FacetFilter
     * @extends sap.ui.core.Control
     */
    sap.ui.core.Control.extend("com.zespri.awct.control.FacetFilter", /** @lends com.zespri.awct.control.FacetFilter */
    {
        metadata : {
            ...
            properties : {
                ...
                "type" : {
                    ...
                    group : "Appearance",
                    defaultValue : sap.m.FacetFilterType.Simple
                },
                "liveSearch" : {
                    type : "boolean",
                    group : "Behavior",
                ...
                // info text to display as tooltip when control is disabled
                "disabledText" : {
                    type : "string",
                    defaultValue : ''
                }
            ...
            aggregations : {
                "lists" : {
                    type : "com.zespri.awct.control.FacetFilterList",
                    multiple : true,
                    singularName : "list"
                },
                "buttons" : {
        ...
        renderer : "com.zespri.awct.control.FacetFilterRenderer"
    });

    com.zespri.awct.control.FacetFilter.M_EVENTS = {
        'reset' : 'reset'
    };
    ...
    sap.ui.core.EnabledPropagator.call(com.zespri.awct.control.FacetFilter.prototype);
    // CUSTOM CODE END ---

    // How many pixels to scroll with every overflow arrow click
    com.zespri.awct.control.FacetFilter.SCROLL_STEP = 264;

    com.zespri.awct.control.FacetFilter.prototype.setType = function(oType) {
    ...
    com.zespri.awct.control.FacetFilter.prototype.setShowReset = function(bVal) {
    ...
    com.zespri.awct.control.FacetFilter.prototype.setShowSummaryBar = function(bVal) {
    ...
    com.zespri.awct.control.FacetFilter.prototype.setLiveSearch = function(bVal) {
    ...
    com.zespri.awct.control.FacetFilter.prototype.getLists = function() {
    ...
    com.zespri.awct.control.FacetFilter.prototype.removeList = function(vObject) {
    ...
    com.zespri.awct.control.FacetFilter.prototype.removeAggregation = function() {
    ...
    com.zespri.awct.control.FacetFilter.prototype.openFilterDialog = function() {
    ...
    /**
     * Gets the css width of the facet filter. (not the effective width). Even though there is no width property defined for the FacetFilter, this
     * method is needed for proper display in the ToolBar.
     * 
     * @returns {string} the width as a string
     * @protected
     */
    com.zespri.awct.control.FacetFilter.prototype.getWidth = function() {
        return "100%";
    };
     ...
     * @private
     */
    com.zespri.awct.control.FacetFilter.prototype.init = function() {
     ...
     * @private
     */
    com.zespri.awct.control.FacetFilter.prototype.exit = function() {
     ...
     * @private
     */
    com.zespri.awct.control.FacetFilter.prototype.onBeforeRendering = function() {
    ...
    /**
     * Attach a interval timer that periodically checks overflow of the "head" div in the event that the window is resized or the device orientation
     * is changed. This is ultimately to see if carousel arrows should be displayed.
     * 
     * @private
     */
    com.zespri.awct.control.FacetFilter.prototype.onAfterRendering = function() {
    ...
    /**
     * Get the facet popover displayed when the user presses the facet button (Simple type only). The popover is created if it does not exist and is
     * available via the "popover" aggregation. This aggregation is destroyed when the popover is closed.
     * 
     * @returns {sap.m.Popover} Multiple calls return the same popover instance.
     * @private
     */
    com.zespri.awct.control.FacetFilter.prototype._getPopover = function() {
        ...
        if (!oPopover) {
            ...
            oPopover = new sap.m.Popover({
                ...
                afterClose : function() {
                    ...
                    if (com.zespri.awct.util.Constants.C_IS_BROWSER_IE && sap.ui.Device.browser.version < 10) {
     ...
     * @private
     */
    com.zespri.awct.control.FacetFilter.prototype._handlePopoverAfterClose = function(oList) {
        this._displayRemoveIcon(false, oList);
        oList._fireListCloseEvent();
        // Destroy the popover aggregation, otherwise if the list is then moved to the dialog filter items page, it will still think it's DOM element
        // parent
     ...
     * @param oPopover
     * @param oControl
     *            The control the popover will be opened "by"
     * 
     * 
     */
    com.zespri.awct.control.FacetFilter.prototype._openPopover = function(oPopover, oControl) {
        ...
        if (!oPopover.isOpen()) {
            ...
            // CUSTOM CODE START ---
            // The below code (present in standard implementation) is commented because it is not needed. It causes refresh issues on the UI for a
            // certain case : 1. Open a facet filter list (dependent). 2. Enter a search term. 3. Close the facet list. 4. Open the same list. Since
            // it is a dependent filter, the values will be loaded again.. but 'busy' state will not be visible as _applySearch() will affect how
            // CommonHelper.manageNoDataText() works, and will remove busy state sooner than expected.
            // oList._applySearch();
            // --- CUSTOM CODE END
        }
     ...
     * @private
     */
    com.zespri.awct.control.FacetFilter.prototype._getAddFacetButton = function() {

        var oButton = this.getAggregation("addFacetButton");
        if (!oButton) {
            var that = this;
            var sButtonTooltip = com.zespri.awct.util.I18NHelper.getText("TXT_FACET_FILTER_MORE_TOOLTIP");
            oButton = new sap.m.Button(this.getId() + "-add", {
     ...
     * 
     * @param oList
     *            The list displayed when the button is pressed
     * @returns {sap.m.Button}
     * @private
     */
    com.zespri.awct.control.FacetFilter.prototype._getButtonForList = function(oList) {

        if (this._buttons[oList.getId()]) {
            // CUSTOM CODE START ---
            // Everytime the filter is shown, the buttons are not created again, instead the existing buttons are only used.
            // The 'enabled' property is set to the buttons of the facet filter.
            var bEnabled = oList.getEnabled();
            this._buttons[oList.getId()].setEnabled(bEnabled);
            // --- CUSTOM CODE END
        ...
        // First time when the facet filter is loaded, in the order history screen, delivery line number list is disabled. Hence, the 'enabled'
        // property of the buttons also has to be set.
        var that = this;
        var oButton = new sap.m.Button({
            // CUSTOM CODE START ---
            enabled : oList.getEnabled(),
            // --- CUSTOM CODE END
            type : sap.m.ButtonType.Transparent,
            press : function() {
     ...
     * @private
     */
    com.zespri.awct.control.FacetFilter.prototype._setButtonText = function(oList) {
        ...
        if (oButton) { // Button may not be created yet if FFL.setTitle() is called before the button is rendered the first time
            ...
            if (iLength > 0) {
                // CUSTOM CODE START----
                var sFilterText = (iLength === 1) ? oList._oSelectedKeys[aSelectedKeyNames[0]] : iLength;
                sText = this._bundle.getText("FACETFILTER_ITEM_SELECTION", [oList.getTitle(), sFilterText]);
                // ---- CUSTOM CODE END
            } else {
    ...
    /**
     * Get the facet list remove icon for the given list (it is created if it does not yet exist). The icon is associated with the list id, which is
     * why we only need to pass the list to retrieve the icon once it has been created.
     * 
     * @private
     */
    com.zespri.awct.control.FacetFilter.prototype._getFacetRemoveIcon = function(oList) {
     ...
     * @private
     */
    com.zespri.awct.control.FacetFilter.prototype._displayRemoveIcon = function(bDisplay, oList) {
    ...
    /**
     * Create the navigation container displayed in the facet dialog. The container is created with an initial page for the list of facets and a
     * second page for displaying a list of items associated with the facet selected on the initial page.
     * 
     * @private
     */
    com.zespri.awct.control.FacetFilter.prototype._getFacetDialogNavContainer = function() {
        ...
        oNavContainer.attachAfterNavigate(function(oEvent) {
            ...
            if (oToPage === oFacetPage) {
                ...
                jQuery.sap.assert(that._displayedList === null,
                        "Filter items list should have been placed back in the FacetFilter aggregation before page content is destroyed.");
    ...
    /**
     * Create a page that contains a list of facets and a search field for searching facets. Each facet represents one FacetFilterList.
     * 
     * @returns {sap.m.Page}
     * @private
     */
    com.zespri.awct.control.FacetFilter.prototype._createFacetPage = function() {
        ...
        var oPage = new sap.m.Page({
            ...
            title : com.zespri.awct.util.I18NHelper.getText("TXT_FACETFILTER_DIALOG_TITLE_TEXT"),
     ...
     * @private
     */
    com.zespri.awct.control.FacetFilter.prototype._createFilterItemsPage = function() {
    ...
    /**
     * Create a new page that contains a FacetFilterList and a search field for searching items. Old page is destroyed.
     * 
     * @returns {sap.m.Page}
     * @private
     */
    com.zespri.awct.control.FacetFilter.prototype._getFilterItemsPage = function(oNavCont) {
     ...
     * @private
     */
    com.zespri.awct.control.FacetFilter.prototype._createFilterItemsSearchFieldBar = function(oList) {

        // CUSTOM CODE START ---
        // No need for a search field for our custom FacetFilterDateInputList, FacetFilterCheckboxList and FacetFilterTextInputList controls
        if ((oList instanceof com.zespri.awct.control.FacetFilterDateInputList) ||
                (oList instanceof com.zespri.awct.control.FacetFilterNumberInputList) ||
                (oList instanceof com.zespri.awct.control.FacetFilterCheckboxList)) {
            return null;
        }
        // --- CUSTOM CODE END
        ...
        // CUSTOMC CODE START ---
        if (oList._bPendingClearSearchField) {
            oSearchField.setValue();
            oList._bPendingClearSearchField = false;
        }
        oSearchField.setEnabled(oList._bSearchFieldEnabled);
        // -- CUSTOM CODE END
        return oBar;
    };

    /**
     * Get the facet dialog. The dialog is created if it doesn't exist. The dialog contains a NavContainer having two Pages. The first page contains a
     * list of facets. Navigation proceeds to a second page containing facet filter items for the selected facet.
     * 
     * @private
     */
    com.zespri.awct.control.FacetFilter.prototype._getFacetDialog = function() {
     ...
     * @private
     */
    com.zespri.awct.control.FacetFilter.prototype._closeDialog = function() {
        var oDialog = this.getAggregation("dialog");
     ...
     * @private
     */
    com.zespri.awct.control.FacetFilter.prototype._closePopover = function() {
     ...
     * 
     * @returns {sap.m.List} A list populated with items, each displaying a title (from the FacetFilterList title) and a counter (from the
     *          FacetFilterList allCount)
     * @private
     */
    com.zespri.awct.control.FacetFilter.prototype._createFacetList = function() {
        ...
        com.zespri.awct.util.CommonHelper.manageNoDataText(oFacetList);
        // CUSTOM CODE END
    ...
    /**
     * Create a bar containing a "select all" checkbox for the given list. The "checkbox" association is created from the list to the checkbox so that
     * the checkbox selected state can be updated by the list when selection changes.
     * 
     * @param oList
     * @returns {sap.m.Bar} Bar, or null if the given list is not multi-select
     * @private
     */
    com.zespri.awct.control.FacetFilter.prototype._createSelectAllCheckboxBar = function(oList) {

        // CUSTOM CODE START ---
        // We don't need a 'Select all' checkbox bar for our custom FacetFilterDateInputList, FacetFilterCheckboxList and FacetFilterTextInputList
        // controls
        if ((oList instanceof com.zespri.awct.control.FacetFilterDateInputList) ||
                (oList instanceof com.zespri.awct.control.FacetFilterNumberInputList) ||
                (oList instanceof com.zespri.awct.control.FacetFilterCheckboxList)) {
        ...
        var fnHandleCheckboxSelection = function(bSelected) {
            ...
            // CUSTOM CODE START ---
            // If selection of the 'All' checkbox changed, set the filter as dirty
            var oFilter = sap.ui.getCore().byId(oList.getAssociation("facetFilter"));
            oFilter.setProperty("filtersModifiedAfterListOpen", true, true);
            // -- CUSTOM CODE END
        };
     ...
     * @private
     */
    com.zespri.awct.control.FacetFilter.prototype._handleFacetListItemPress = function(oEvent) {
     ...
     * 
     * @param oFacetListItem
     *            The facet item selected on the dialog facet page
     * @private
     */
    com.zespri.awct.control.FacetFilter.prototype._navToFilterItemsPage = function(oFacetListItem) {
        ...
        if (this._listIndexAgg === iIndex) {
            ...
            // CUSTOM CODE START
            oFilterItemsPage.setTitle(com.zespri.awct.util.I18NHelper.getText("TXT_FACETFILTER_DIALOG_TITLE_TEXT") + ": " +
                    oFacetFilterList.getTitle());
            // CUSTOM CODE END
     ...
     * @private
     */
    com.zespri.awct.control.FacetFilter.prototype._navFromFilterItemsPage = function(oNavContainer) {
     ...
     * @private
     */
    com.zespri.awct.control.FacetFilter.prototype._moveListToDisplayContainer = function(oList, oContainer) {
     ...
     * @private
     */
    com.zespri.awct.control.FacetFilter.prototype._restoreListFromDisplayContainer = function(oContainer) {
    ...
    /**
     * Returns an array in ascending according to the sequence value of each FacetFilterList. If a list has sequence <= -1 then its sequence is reset
     * to its index in the "lists" aggregation.
     * 
     * @returns Sorted list of FacetFilterLists.
     * @private
     */
    com.zespri.awct.control.FacetFilter.prototype._getSequencedLists = function() {
     ...
     * @private
     */
    com.zespri.awct.control.FacetFilter.prototype._getSummaryBar = function() {
     ...
     * @private
     */
    com.zespri.awct.control.FacetFilter.prototype._createResetButton = function() {

        var that = this;
        var oButton = new sap.m.Button({
            type : sap.m.ButtonType.Transparent,
            icon : sap.ui.core.IconPool.getIconURI("undo"),
            tooltip : com.zespri.awct.util.I18NHelper.getText("TXT_GENERIC_RESET"),
            press : function() {

                that.fireReset();

                // CUSTOM CODE START ---
                // During reset, also reset the 'filtersModifiedAfterListOpen' property which indicates whether the user has changed any filter
                // selections.
                that.setProperty("filtersModifiedAfterListOpen", false, true);
                // --- CUSTOM CODE END
     ...
     * @param oPopover
     */
    com.zespri.awct.control.FacetFilter.prototype._addOKButtonToPopover = function(oPopover) {
     ...
     * @private
     */
    com.zespri.awct.control.FacetFilter.prototype._getSummaryText = function() {
     ...
     * @private
     */
    com.zespri.awct.control.FacetFilter.prototype._getSelectedItemsText = function(oList) {

        var aTexts = oList.getSelectedItems().map(function(value) {
            return value.getText();
        });

        if (oList._oSelectedKeys) {
            Object.getOwnPropertyNames(oList._oSelectedKeys).forEach(function(value) {
                if (aTexts.indexOf(oList._oSelectedKeys[value]) === -1) {
                    aTexts.push(oList._oSelectedKeys[value]);
                }
            });
        }
        return aTexts;
     ...
     * @private
     */
    com.zespri.awct.control.FacetFilter.prototype._addResetToSummary = function(oSummaryBar) {

        if (oSummaryBar.getContent().length === 1) {

            oSummaryBar.addContent(new sap.m.ToolbarSpacer({
                width : ""
            })); // Push the reset button to the end of the toolbar
     ...
     * @private
     */
    com.zespri.awct.control.FacetFilter.prototype._removeResetFromSummary = function(oSummaryBar) {
     ...
     * @param oList
     */
    com.zespri.awct.control.FacetFilter.prototype._removeList = function(oList) {
     ...
     */
    com.zespri.awct.control.FacetFilter.prototype._getScrollingArrow = function(sName) {
        ...
        if (sName === "left") {
            ...
            if (!oArrowIcon) {
                ...
                for ( var i = 0; i < aCssClassesToAddLeft.length; i++) {
                    ...
                    var sPreviousTooltip = com.zespri.awct.util.I18NHelper.getText("TXT_FACET_FILTER_PREVIOUS_TOOLTIP");
                    oArrowIcon.setTooltip(sPreviousTooltip);
                }
            ...
            if (!oArrowIcon) {
                ...
                for ( var j = 0; j < aCssClassesToAddRight.length; j++) {
                    oArrowIcon.addStyleClass(aCssClassesToAddRight[j]);
                    var sNextTooltip = com.zespri.awct.util.I18NHelper.getText("TXT_FACET_FILTER_NEXT_TOOLTIP");
                    oArrowIcon.setTooltip(sNextTooltip);
                }
     ...
     * @private
     */
    com.zespri.awct.control.FacetFilter.prototype._checkOverflow = function() {
     ...
     * @private
     */
    com.zespri.awct.control.FacetFilter.prototype.onclick = function(oEvent) {
        ...
        if (sTargetId) {
            ...
            if (sTargetId === sId + "-arrowScrollLeft") {
                ...
                this._scroll(-com.zespri.awct.control.FacetFilter.SCROLL_STEP, 500);
     ...
     * @private
     */
    com.zespri.awct.control.FacetFilter.prototype._scroll = function(iDelta, iDuration) {

        var oDomRef = this.getDomRef("head");
        var iScrollLeft = oDomRef.scrollLeft;
        if (!!!com.zespri.awct.util.Constants.C_IS_BROWSER_IE && this._bRtl) {
            iDelta = -iDelta;
     ...
     * @private
     */
    com.zespri.awct.control.FacetFilter.prototype._enableTouchSupport = function() {
        ...
        var fnTouchEnd = function(evt) {
            ...
            if (that._bTouchNotMoved === false) { // swiping ends now
                ...
                var oListRef = that.getDomRef("head");
                var dt = 50;
                var endVelocity = Math.abs(that._velocity / 10); // continue scrolling until the speed has decreased to a fraction (v/10 means 11
                // iterations with slowing-down factor
                // 0.8)
